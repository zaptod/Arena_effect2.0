def show_agents_carousel(screen):
	...
	while running:
		...
		txt = font.render('Criar Novo Agente', True, (255,255,255))
		...
		end = min(len(agentes_db), carousel_start + visible_count)
		for idx, agent in enumerate(agentes_db[carousel_start:end]):
			y = 100 + idx*90
			rect = pygame.Rect(150, y, 500, 80)
			color = (0,200,0) if agent.get('ativo', True) else (200,0,0)
			pygame.draw.rect(screen, color, rect)
			txt = font.render(f"{agent['nome']} (ID: {agent['id']})", True, (255,255,255))
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if event.key == pygame.K_RIGHT and carousel_start + visible_count < len(agentes_db):
					carousel_start += 1
				...
				if btn_novo.collidepoint(event.pos):
					popup_criar_agente(screen)
					recarregar_agentes()
				if btn_left.collidepoint(event.pos) and carousel_start > 0:
					carousel_start -= 1
				if btn_right.collidepoint(event.pos) and carousel_start + visible_count < len(agentes_db):
					carousel_start += 1
				for idx, agent in enumerate(agentes_db[carousel_start:end]):
					y = 100 + idx*90
					rect = pygame.Rect(150, y, 500, 80)
					...
					if btn_del.collidepoint(event.pos):
						nome_arquivo = f"agente{agent['id']}.py"
						caminho = os.path.join(os.path.dirname(__file__), '..', 'agentes', nome_arquivo)
						if os.path.exists(caminho):
							os.remove(caminho)
						recarregar_agentes()
						if carousel_start > 0 and carousel_start + visible_count > len(agentes_db):
							carousel_start -= 1
						break
					if rect.collidepoint(event.pos):
						from Paginas.AgenteEdit import edit_agent
						edit_agent(agent, screen)
						recarregar_agentes()
		pygame.display.flip()
		clock.tick(60)
import pygame
...
def recarregar_agentes():
	global agentes_db
	import importlib
	banco_mod = importlib.import_module('bases_de_dados.Banco_de_dados_agentes')
	importlib.reload(banco_mod)
	agentes_db = banco_mod.agentes_db

def popup_criar_agente(screen):
	font = pygame.font.SysFont(None, 28)
	...
	while running_popup:
		popup.fill((60,60,80))
		popup.blit(font.render('Criar Novo Agente', True, (255,255,255)), (140, 20))
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if btn_confirm.collidepoint(event.pos):
					novo_id = max([a['id'] for a in agentes_db]+[0]) + 1
					nome_arquivo = f"agente{novo_id}.py"
					caminho = os.path.join(os.path.dirname(__file__), '..', 'agentes', nome_arquivo)
					agent_code = (
						"import pygame\n"
						"class Agente:\n"
