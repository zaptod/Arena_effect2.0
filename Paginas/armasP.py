...
def show_armas_carousel(screen):
	...
	while running:
		...
		# Título
		screen.blit(font.render('Armas Disponíveis', True, (255,255,255)), (250, 30))
		...
		# Lista de armas
		end = min(len(armas_db), carousel_start + visible_count)
		for idx, arma in enumerate(armas_db[carousel_start:end]):
			y = 100 + idx*90
			rect = pygame.Rect(150, y, 500, 80)
			pygame.draw.rect(screen, (70,130,180), rect)
			
			txt = font.render(f"{arma['nome']} (ID: {arma['id']})", True, (255,255,255))
			screen.blit(txt, (rect.x+10, rect.y+10))
			
			txt_info = font.render(f"Dano: {arma['dano']} | Alcance: {arma['alcance']} | Vel: {arma['velocidade_ataque']}", True, (200,200,200))
			screen.blit(txt_info, (rect.x+10, rect.y+40))
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if event.key == pygame.K_RIGHT and carousel_start + visible_count < len(armas_db):
					carousel_start += 1
				...
				if btn_left.collidepoint(event.pos) and carousel_start > 0:
					carousel_start -= 1
				if btn_right.collidepoint(event.pos) and carousel_start + visible_count < len(armas_db):
					carousel_start += 1
